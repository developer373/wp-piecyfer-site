...
/**
 * Wraps the `aioseo_blc_migrations_log` option that tracks per-migration
 * execution state for the {@see MigrationRunner}.
 *
 * The option is stored without autoload — healthy sites short-circuit on
 * `lastSchemaVersion === aioseoBrokenLinkChecker()->version` before the runner
 * ever reads the log, so paying autoload overhead on every request would be
 * wasted work.
 *
 * @since 1.3.0
 */
class MigrationLog {
	 ...
	 * @var string
	 */
	private $optionName = 'aioseo_blc_migrations_log';
