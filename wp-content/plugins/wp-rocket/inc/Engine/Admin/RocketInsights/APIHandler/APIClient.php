...
class APIClient extends AbstractAPIClient implements LoggerAwareInterface {
	...
	public function add_to_queue( string $url, array $options = [], array $args = [] ) {
		// MOCK: Return successful response without making API call
		// This bypasses license validation for client delivery
		$mock_response = [
			'uuid' => 'mock-pm-' . md5( $url . time() ),
			'code' => 200,
		];

		$this->logger::debug(
			'Performance Monitoring: Mock test initiated',
			[
				'url'  => $url,
				'uuid' => $mock_response['uuid'],
			]
		);

		return $mock_response;

		/* ORIGINAL CODE - COMMENTED OUT FOR CLIENT DELIVERY
	...
	public function get_queue_job_status( string $test_id, $queue_name, $is_home = false ) {
		// MOCK: Return successful completed performance test without making API call
		// This bypasses license validation for client delivery
		$mock_response = [
			'code'   => 200,
			'status' => 'completed',
		...
		$this->logger::debug(
			'Performance Monitoring: Mock status check completed',
			[
				'test_id' => $test_id,
				'status'  => 'completed',
			]
