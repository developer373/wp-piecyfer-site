...
class UpdraftPlus_CLI_Command extends UpdraftPlus_CLI_Command_Base {
		...
		if ('' != $only_these_table_entities) {
			...
			foreach ($temp_onlythesetableentities as $onlythesetableentity) {
				if (0 === stripos($onlythesetableentity, $wpdb->prefix)) {
					$query = $wpdb->prepare("SHOW TABLES LIKE %s", UpdraftPlus_Database_Utility::esc_like($onlythesetableentity));
	...
	public function search_and_replace($args, $assoc_args) {
		...
		if (!class_exists('UpdraftPlus_Addons_Migrator')) $this->addon_not_exist_error('migration', 'Migrator', 'https://teamupdraft.com/updraftplus/wordpress-migration-plugin/');
	...
	public function restore($args, $assoc_args) {
		...
		if (class_exists('UpdraftPlus_Addons_Migrator')) {
			...
			if (isset($assoc_args['migration'])) {
				$this->addon_not_exist_error('migration', 'Migrator', 'https://teamupdraft.com/updraftplus/wordpress-migration-plugin/');
			}
		}
	...
	private function init_wp_filesystem() {
		...
		if ($wp_filesystem->errors->get_error_code()) {
			$url = apply_filters('updraftplus_com_link', "https://teamupdraft.com/documentation/updraftplus/topics/restoration/troubleshooting/why-am-i-being-asked-for-ftp-details-restoration-migration-plugin-installation-update/").'">';
