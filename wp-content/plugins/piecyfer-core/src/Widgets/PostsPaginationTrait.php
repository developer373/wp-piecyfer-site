<?php
/**
 * Port of Elementor Pro's `ElementorPro\Modules\Posts\Traits\Pagination_Trait`.
 *
 * `Posts_Base` itself only uses two of these methods — `get_base_url()` and
 * `is_posts_page()` — but the rest are part of the trait's public surface and
 * are what Pro's `Posts\Module` and `ThemeBuilder\Locations_Manager` call to
 * decide whether a paged URL such as `/blogs/2/` is a real page or a 404.
 *
 * That 404 decision is **not** wired up by this file. Pro's `Posts\Module`
 * hooks `pre_handle_404` and calls is_valid_pagination() with the document's
 * element data; nothing in piecyfer-core does that yet, so once Elementor Pro
 * is removed `/blogs/2/` will 404 unless a module-level equivalent is added.
 * The methods are ported here so that module has something to call.
 *
...
trait PostsPaginationTrait {

	/**
	 * Whether a set of elements contains a posts/archive widget that can be
	 * paginated to the given page number.
	 *
	...
	/**
	 * Every widget name that may add pagination.
	 *
	 * Pro also counts `Global_Widget` here; we have no global-widget equivalent,
	 * so the check is limited to our own posts base class.
	 *
	 * @return string[]
	 */
	public function get_widgets_that_support_pagination() {
		...
		foreach ( $widgets as $widget ) {
			if ( $widget instanceof PostsBaseWidget ) {
	...
	/**
	 * Whether the query is for an existing blog posts index page.
	 *
