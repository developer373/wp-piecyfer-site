<?php
/**
 * Port of `ElementorPro\Modules\Posts\Skins\Skin_Base`
 * (elementor-pro/modules/posts/skins/skin-base.php, 1409 lines).
 *
 * Everything a posts skin has in common: the Layout-section controls, the three
 * Style sections, and the render loop. Subclasses change the id, the container
 * class and — in VamTam's case — the order of the parts inside one post item.
 *
 * ## Why the control ids matter more than the markup
 *
 * `Elementor\Skin_Base::get_control_id()` prefixes every id registered from a
 * skin with `str_replace( '-', '_', $this->get_id() ) . '_'`, so `meta_data`
 * registered here becomes `vamtam_classic_meta_data` in the widget's stack and
 * in `_elementor_data`. 91 saved keys per `posts` instance depend on that
 * prefix landing exactly right, and 46 of them are typography sub-fields whose
 * concrete values are empty strings — the real values live in `__globals__`,
 * keyed by control id. Get one id wrong and the global reference orphans with
 * no fallback value behind it: unstyled headings across the blog, no error
 * anywhere. Nothing in this file may be "optimised away" for looking empty.
 *
 * ## Whitespace
 *
 * The `?>` … `<?php` blocks below emit their leading tabs and newlines as
 * literal output. They are copied from Pro at the same indentation depth
 * (methods at one tab, template bodies at two) and must stay there.
 *
...
use PieCyfer\Core\Widgets\PostsBaseWidget;
use PieCyfer\Core\Widgets\PostsButtonTrait;
...
abstract class SkinBase extends Elementor_Skin_Base {
	...
	/**
	 * The permalink is captured once per post so a plugin that filters
	 * `post_link` mid-item cannot make the thumbnail and the title disagree.
	 *
	 * @var string|null
	 */
	...
	protected function register_thumbnail_controls() {
		$this->add_control(
			'thumbnail',
			[
				'label' => esc_html__( 'Image Position', 'piecyfer-core' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'top',
				'options' => [
					'top' => esc_html__( 'Top', 'piecyfer-core' ),
					'left' => esc_html__( 'Left', 'piecyfer-core' ),
					'right' => esc_html__( 'Right', 'piecyfer-core' ),
		...
		$this->add_control(
			'masonry',
			[
				'label' => esc_html__( 'Masonry', 'piecyfer-core' ),
		...
		$this->add_responsive_control(
			...
			[
				...
				/*
				 * The `:after { content }` rule is not decorative — it is how the
				 * front-end handler reads the ratio back out of the computed
				 * style before adding `elementor-has-item-ratio`. That class is
				 * added by JavaScript, never by PHP.
				 */
				'selectors' => [
	...
	protected function register_post_count_control() {
		$this->add_control(
			'posts_per_page',
			[
				'label' => esc_html__( 'Posts Per Page', 'piecyfer-core' ),
	...
	protected function register_excerpt_controls() {
		$this->add_control(
			'show_excerpt',
			[
				'label' => esc_html__( 'Excerpt', 'piecyfer-core' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'Show', 'piecyfer-core' ),
				'label_off' => esc_html__( 'Hide', 'piecyfer-core' ),
				'default' => 'yes',
		...
		$this->add_control(
			'excerpt_length',
			[
				'label' => esc_html__( 'Excerpt Length', 'piecyfer-core' ),
				'type' => Controls_Manager::NUMBER,
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Apply to custom Excerpt', 'piecyfer-core' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'Yes', 'piecyfer-core' ),
				'label_off' => esc_html__( 'No', 'piecyfer-core' ),
				'default' => 'no',
				'condition' => [
					$this->get_control_id( 'show_excerpt' ) => 'yes',
				],
	...
	protected function register_read_more_controls() {
		$this->add_control(
			'show_read_more',
			[
				'label' => esc_html__( 'Read More', 'piecyfer-core' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'Show', 'piecyfer-core' ),
				'label_off' => esc_html__( 'Hide', 'piecyfer-core' ),
				'default' => 'yes',
		...
		$this->add_control(
			'read_more_text',
			[
				'label' => esc_html__( 'Read More Text', 'piecyfer-core' ),
				...
				'default' => esc_html__( 'Read More »', 'piecyfer-core' ),
				'condition' => [
		...
		$this->add_control(
			'read_more_alignment',
			[
				'label' => esc_html__( 'Automatically align buttons', 'piecyfer-core' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'Yes', 'piecyfer-core' ),
				'label_off' => esc_html__( 'No', 'piecyfer-core' ),
				'default' => '',
	...
	protected function register_link_controls() {
		$this->add_control(
			'open_new_tab',
			[
				'label' => esc_html__( 'Open in new window', 'piecyfer-core' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'Yes', 'piecyfer-core' ),
				'label_off' => esc_html__( 'No', 'piecyfer-core' ),
				'default' => 'no',
	...
	protected function register_meta_data_controls() {
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'modified' => esc_html__( 'Date Modified', 'piecyfer-core' ),
				],
				'separator' => 'before',
			]
		);
		...
		$this->add_control(
			'meta_separator',
			[
				'label' => esc_html__( 'Separator Between', 'piecyfer-core' ),
				'type' => Controls_Manager::TEXT,
				'default' => '///',
				'ai' => [
					'active' => false,
	...
	protected function register_design_content_controls() {
		...
		$this->add_control(
			'heading_meta_style',
			[
				'label' => esc_html__( 'Meta', 'piecyfer-core' ),
				'type' => Controls_Manager::HEADING,
				'separator' => 'before',
				'condition' => [
					$this->get_control_id( 'meta_data!' ) => [],
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Separator Color', 'piecyfer-core' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
	 ...
	 *
	 * `AbstractWidget::render()` is `final protected` and is never called for a
	 * skinned widget — `Widget_Base::render_content()` dispatches straight to
	 * the skin — so the plugin's "one broken widget must not blank the page"
	 * guard has to live here instead. Subclasses override `render_skin()`, which
	 * is this class's only intentional divergence from Pro's shape (Pro has no
	 * guard and lets a throw take the page down).
	 */
	...
	protected function render_failure( \Throwable $e ): void {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
				sprintf( '[piecyfer-core] posts skin %s render failed: %s in %s:%d', $this->get_id(), $e->getMessage(), $e->getFile(), $e->getLine() )
			);
		}
		...
		printf(
			...
			sprintf(
				/* translators: 1: skin id, 2: error message */
				esc_html__( 'PieCyfer posts skin "%1$s" failed to render: %2$s', 'piecyfer-core' ),
	...
	/**
	 * Pro's `Skin_Base::render()`, verbatim.
	 */
	...
	protected function render_excerpt() {
		/*
		 * The double add_filter() and the early return between them are Pro's,
		 * and they are a bug: when `show_excerpt` is off the filters are added
		 * and never removed, so they leak into whatever renders next. Every live
		 * instance on this site leaves `show_excerpt` unset (Pro default `yes`),
		 * so the leak path is never taken — but "fixing" it here would change
		 * behaviour that has never been captured. Reproduced as-is.
		 */
	...
	/**
	 * Stand-in for `ElementorPro\Core\Utils::trim_words()`.
	 *
	 * @param string $text
	 * @param int    $length
	 * @return string
	 */
	protected static function trim_words( $text, $length ) {
		if ( ! $length ) {
			return $text;
		}

		$whitespace_pattern = '/\s+/u';
		$words = preg_split( $whitespace_pattern, (string) $text, -1, PREG_SPLIT_NO_EMPTY );
	...
	protected function render_read_more() {
		...
		$aria_label_text = sprintf(
			...
			esc_attr__( 'Read more about %s', 'piecyfer-core' ),
			get_the_title()
		);
	...
	protected function render_post_header() {
		/*
		 * `post_class( [ 'elementor-post elementor-grid-item' ] )` — one string,
		 * exactly as Pro passes it. The order is load-bearing: VamTam's
		 * `checkDiscardDuplicates()` reads the post id out of `classList[2]`,
		 * which only works while the first two classes are these two.
		 */
		?>
	...
	protected function render_loop_header() {
		...
		/*
		 * Pro also ORs in `Taxonomy_Loop_Provider::is_loop_taxonomy()`, which
		 * belongs to the Loop Builder module. That module is not ported (no loop
		 * grid exists on this site) and the check can only ever *add* the class
		 * — `found_posts` is set on every WP_Query, so the first operand is
		 * already true here.
		 */
		if ( isset( $e_wp_query->found_posts ) ) {
	...
	protected function render_message() {
		/*
		 * Pro reads `load_more_no_posts_custom_message` unconditionally, ignoring
		 * `load_more_no_posts_message_switcher`. On this site the switcher is
		 * never set, so `.e-load-more-message` renders empty — which is what the
		 * baseline shows. Do not add the switcher check.
		 */
		$settings = $this->parent->get_settings_for_display();
		?>
	...
	protected function render_loop_footer() {
		...
		$using_ajax_pagination = in_array( $parent_settings['pagination_type'], [
			PostsBaseWidget::LOAD_MORE_ON_CLICK,
			PostsBaseWidget::LOAD_MORE_INFINITE_SCROLL,
		], true);
		...
		/*
		 * Everything below is numbered / prev-next pagination. No instance on
		 * this site uses it (all six paginating widgets are load_more_on_click),
		 * so it is ported but UNTESTED against a capture.
		 */
		$links = [];
		...
		if ( $has_numbers ) {
			$paginate_args = [
				...
				'before_page_number' => '<span class="elementor-screen-only">' . esc_html__( 'Page', 'piecyfer-core' ) . '</span>',
			];
		...
		?>
		<nav class="elementor-pagination" aria-label="<?php esc_attr_e( 'Pagination', 'piecyfer-core' ); ?>">
	...
	/**
	 * Whether the Read More link is wrapped and pushed to the bottom of the item.
	 *
	 * Conditions: read-more aligned to the bottom, no masonry, read-more shown.
	 */
