<?php
/**
 * Port of `ElementorPro\Modules\Posts\Skins\Skin_Cards`
 * (elementor-pro/modules/posts/skins/skin-cards.php).
 *
 * Dead on this site — two orphaned `cards_*` keys survive in `_elementor_data`
 * and nothing selects the skin. Registered so the `posts` skin bucket is
 * complete and deterministic, and so those keys keep a matching control.
 *
...
class CardsSkin extends SkinBase {
	...
	/*
	 * Pro re-declares the four tab helpers here. `$args` in start_controls_tabs()
	 * is written to and then never used — reproduced so the class shape matches,
	 * because the *effect* (tab ids still going through get_control_id()) is
	...
	protected function register_thumbnail_controls() {
		...
		$this->update_control(
			'thumbnail',
			[
				'label' => esc_html__( 'Show Image', 'piecyfer-core' ),
				'options' => [
					'top' => esc_html__( 'Yes', 'piecyfer-core' ),
					'none' => esc_html__( 'No', 'piecyfer-core' ),
				],
	...
	public function register_badge_controls() {
		...
		$this->add_control(
			'badge_taxonomy',
			[
				'label' => esc_html__( 'Badge Taxonomy', 'piecyfer-core' ),
	...
	public function register_design_card_controls() {
		$this->start_controls_section(
			'section_design_card',
			[
				'label' => esc_html__( 'Card', 'piecyfer-core' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Box Shadow', 'piecyfer-core' ),
				'type' => Controls_Manager::SWITCHER,
		...
		$this->add_control(
			'hover_effect',
			[
				'label' => esc_html__( 'Hover Effect', 'piecyfer-core' ),
				'type' => Controls_Manager::SELECT,
				'label_block' => false,
				'options' => [
					'none' => esc_html__( 'None', 'piecyfer-core' ),
					'gradient' => esc_html__( 'Gradient', 'piecyfer-core' ),
				],
				'default' => 'gradient',
				'separator' => 'before',
		...
		$this->add_control(
			'meta_border_color',
			[
				'label' => esc_html__( 'Meta Border Color', 'piecyfer-core' ),
				'type' => Controls_Manager::COLOR,
				'separator' => 'before',
				'selectors' => [
	...
	protected function render_avatar() {
		...
		<div class="elementor-post__avatar">
			<?php echo get_avatar( get_the_author_meta( 'ID' ), 128, '', get_the_author_meta( 'display_name' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		</div>
		<?php
	}
