/*!
 * piecyfer-theme front-end bundle.
 *
 * Assembled from the previous theme's Grunt concatenation
 *     lib.js + menu.js + general.js + woocommerce.js + custom-animations.js
 * with two modules deliberately removed:
 *
 *   menu.js        (~2.4 KB) - smooth in-page scrolling bound to
 *                  .vamtam-animated-page-scroll and a mega-menu toggle at
 *                  #main-menu > .mega-menu-wrap. Neither selector occurs on
 *                  any of the 39 captured pages; the live navigation is
 *                  Elementor nav-menu widgets. Its only export,
 *                  VAMTAM.blockStickyHeaderAnimation, is read by
 *                  custom-animations.js and is falsy until menu.js sets it,
 *                  so removing it leaves that module in its normal state.
 *
 *   woocommerce.js (~23 KB, ~45% of the old bundle) - every handler binds to
 *                  WooCommerce selectors or window.wc_add_to_cart_params.
 *                  WooCommerce is not installed.
 *
 * Kept verbatim otherwise, because the public surface
 * (VAMTAM.debounce / isBelowMaxDeviceWidth / isMaxDeviceWidth /
 * adminBarHeight / addScrollHandler) is consumed by piecyfer-core.
 */
...
(function( $, undefined ) {
	...
	*/
	...
	document.addEventListener('DOMContentLoaded', function () {
		// PIECYFER: renamed from low-priority.js to scroll-top.js, which is what
		// it actually is. VAMTAM_FRONT.jspath points at piecyfer-theme/assets/js/.
		window.VAMTAM.load_script( VAMTAM_FRONT.jspath + 'scroll-top.js' );
